...
Imported as a Power Equipment section on each pole. The calculator includes power-owned:

- Streetlights.
- Transformers.
- Risers.

Communication-owned risers are excluded. Equipment heights can lower `Max Height on Pole`. INTEC keeps 40 inches below Low Power, Transformer bottoms, Streetlight drip loops and ungrounded Streetlight brackets; a grounded Streetlight bracket uses 12 inches while its drip loop remains at 40 inches. MidAm applies its dedicated streetlight bracket and uncovered drip-loop rules, while transformers and power risers use Pole Power-comms clearance.

The Equipment table also stores actions: Ground for Streetlights, Redress plus a new HOA for Transformer drip loops, and Raise plus a new HOA for Power Risers. INTEC Streetlights may also be raised independently of Ground, with a maximum increase of one foot from the imported Attachment Height; this moves their bracket and drip-loop clearance references and generates `Raise streetlight from HOA <old> to <new>.` MidAm requires every Streetlight Ground action, so it is enabled automatically and cannot be turned off; its physical bracket and drip-loop clearances still apply. Transformer and Riser work recalculates Low Power when the moved equipment was the limiting power point. Generated equipment instructions use project-specific INTEC or Metronet wording, are included in the pole Make Ready, and survive Save/Load and Update Data.
...
INTEC UG replaces the pole MR with the standard six-line decision block. Pole Actions provides a large `UG Make Ready` editor already populated with that template. Connected Fore/Back/Other Span instructions extract only the reason from its `Unable to attach due to ...` line and format it as `due to on adj pole <reason>`. A Backspan riser is enabled automatically, while the always-available `Riser` action beside UG/PCO can add or remove riser MR manually without requiring an adjacent UG pole. It is placed one foot below the primary Proposed and remains the final Make Ready instruction, including in UG/PCO replacement blocks. Its direction uses the saved user selection first, then imported Make Ready/IO, and finally the primary Proposed/UG span direction for both Fore Span and Back Span. `Riser Direction` remains editable.
