...
INTEC UG replaces the pole MR with the standard six-line decision block. `UG Reason` is editable in Pole Actions and feeds only the reason portion of connected Fore/Back/Other Span UG instructions. Riser direction is model-owned: the calculator reads it from imported Make Ready/IO text and never derives it from the span bearing.
