...
On a normal pole, `Pole Inset` is an independent Make Ready action beside UG, PCO, and Riser. The operator selects either `Overloaded` or `Failing clearances`, and the generated line is `Pole <reason> unless comm inset pole placed at midspan to reduce span length.` UG/PCO replacement Make Ready takes priority over Pole Inset.
