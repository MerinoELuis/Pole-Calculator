...
3. A transfer without HOA Change uses Existing HOA; multiple heights for the same owner produce one ascending-height instruction, with commas and only one final `and` when there are three or more.
